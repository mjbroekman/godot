...
@export var max_wander_range : float = 7.0
...
func navigation_complete():
	...
	if not active and controller.is_stopped:
		return
...
func _new_wander_position ():
	var new_position = get_parent().get_parent().position + _random_offset() * randf_range(0, max_wander_range)
